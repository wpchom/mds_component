...
{
    static const void *mdsFsOpsBegin
        __attribute__((section(MDS_FILE_SYSTEM_SECTION "\000"))) = NULL;
    static const void *mdsFsOpsLimit
        __attribute__((section(MDS_FILE_SYSTEM_SECTION "\177"))) = NULL;
    const MDS_FileSystemOps_t *mdsFsOpsS =
        (const MDS_FileSystemOps_t *)((uintptr_t)(&mdsFsOpsBegin) + sizeof(void *));
    const MDS_FileSystemOps_t *mdsFsOPsE = (const MDS_FileSystemOps_t *)((
        uintptr_t)(&mdsFsOpsLimit));
...
MDS_Err_t MDS_FileOpen(MDS_FileDesc_t *fd, const void *path, MDS_Mask_t flags)
{
    ...
    if ((fileNode->fs == NULL) || (fileNode->fs->ops == NULL) ||
        (fd->node->fs->ops->open == NULL)) {
        return (MDS_EPERM);
    }
...
MDS_Err_t MDS_FileClose(MDS_FileDesc_t *fd)
{
    ...
    if ((fd->node->fs == NULL) || (fd->node->fs->ops == NULL) ||
        (fd->node->fs->ops->close == NULL)) {
        return (MDS_EPERM);
    }
...
MDS_FileOffset_t MDS_FileRead(MDS_FileDesc_t *fd, uint8_t *buff, MDS_FileSize_t len)
{
    ...
    if ((fd->node == NULL) || ((fd->flags & (MDS_FILE_OFLAG_OPEN | MDS_FILE_OFLAG_RDWR)) ==
                               (MDS_FILE_OFLAG_OPEN | MDS_FILE_OFLAG_RDONLY))) {
        return (MDS_EACCES);
    }

    if ((fd->node->fs == NULL) || (fd->node->fs->ops == NULL) ||
        (fd->node->fs->ops->read == NULL)) {
        return (MDS_EPERM);
    }
...
MDS_FileOffset_t MDS_FileWrite(MDS_FileDesc_t *fd, const uint8_t *buff, MDS_FileSize_t len)
{
    ...
    if ((fd->node == NULL) || ((fd->flags & (MDS_FILE_OFLAG_OPEN | MDS_FILE_OFLAG_RDWR)) ==
                               (MDS_FILE_OFLAG_OPEN | MDS_FILE_OFLAG_WRONLY))) {
        return (MDS_EACCES);
    }

    if ((fd->node->fs == NULL) || (fd->node->fs->ops == NULL) ||
        (fd->node->fs->ops->write == NULL)) {
        return (MDS_EPERM);
    }
