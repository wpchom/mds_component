...
static uint16_t EMFS_DataCheckCalculate(uint16_t plus, const uint8_t *buff, size_t len)
{
    ...
    for (size_t idx = 1; idx <= len; idx++) {
        ...
        for (size_t bit = 0; bit < MDS_BITS_OF_BYTE; bit++) {
            crcValue = ((crcValue ^ bitValue) & 0x8000) ? ((crcValue << 1) ^ 0x1021)
                                                        : (crcValue << 1);
            bitValue <<= 1;
        }
    }
...
static size_t EMFS_DeviceSearchPageSize(DEV_STORAGE_Periph_t *device, uint8_t *pageBuff,
                                        size_t buffSize)
{
    size_t pageSize = 0;
    ...
    for (size_t readOfs = 0; readOfs < totalSize; readOfs += EMFS_FS_PAGE_SIZE) {
        ...
        if ((pageSize < EMFS_FS_PAGE_SIZE) || ((pageSize % EMFS_FS_PAGE_SIZE) != 0) ||
            (pageSize > buffSize)) {
            pageSize = 0;
            continue;
        }
...
static MDS_Err_t EMFS_FileSystemReadCheckPage(MDS_EMFS_FileSystem_t *fs, size_t readOfs)
{
    ...
    if ((EMFS_FileSystemJudgMagic(header) == false) ||
        (EMFS_FileSystemGetPageSize(header) > fs->init.size) ||
...
static size_t EMFS_FileSystemGetBlockIndex(DEV_STORAGE_Periph_t *device, size_t blkNums,
                                           size_t offset)
{
    size_t blkOffset = 0;
    size_t blkIndex = 0;
...
static MDS_Err_t EMFS_FileSystemEraseNextPage(MDS_EMFS_FileSystem_t *fs, size_t nextOfs)
{
    ...
    size_t writeBlkE = EMFS_FileSystemGetBlockIndex(fs->init.device, blkNums,
                                                    nextOfs + fs->init.size - 1);

    if (totalSize >= (fs->init.size + fs->init.size)) {
        size_t readBlkS = EMFS_FileSystemGetBlockIndex(fs->init.device, blkNums, fs->readOfs);
        size_t readBlkE = EMFS_FileSystemGetBlockIndex(fs->init.device, blkNums,
                                                       fs->readOfs + fs->init.size - 1);
...
static MDS_Err_t EMFS_FileSystemCheckBlankPage(MDS_EMFS_FileSystem_t *fs, size_t nextOfs)
{
    ...
    while ((err == MDS_EOK) && (ofs < fs->init.size)) {
        size_t read = ((fs->init.size - ofs) > sizeof(buff)) ? (sizeof(buff))
                                                             : (fs->init.size - ofs);
...
static MDS_Err_t EMFS_FileSystemWriteFlush(MDS_EMFS_FileSystem_t *fs)
{
    ...
    for (size_t retry = 0; retry <= MDS_EMFS_WRITE_RETRY; retry++) {
        ...
        err = DEV_STORAGE_PeriphWrite(fs->init.device, fs->writeOfs, fs->init.buff, fs->init.size,
                                      NULL);
        DEV_STORAGE_PeriphClose(fs->init.device);
...
MDS_Err_t MDS_EMFS_Mkfs(DEV_STORAGE_Periph_t *device, size_t pageSize)
{
    ...
    for (size_t ofs = pageSize; (err == MDS_EOK) && (ofs > 0); ofs -= sizeof(buff)) {
        if (ofs > sizeof(buff)) {
            ...
            check = EMFS_DataCheckCalculate(check, buff + sizeof(header->check),
                                            sizeof(buff) - sizeof(header->check));
            EMFS_FileSystemSetCheck(header, check);
        }
...
static EMFS_FileHeader_t *EMFS_FileSystemSearchFile(const MDS_EMFS_FileSystem_t *fs,
                                                    MDS_EMFS_FileId_t id)
{
    EMFS_FsHeader_t *header = (EMFS_FsHeader_t *)(fs->init.buff);
    EMFS_FileHeader_t *find = (EMFS_FileHeader_t *)((uint8_t *)header + sizeof(EMFS_FsHeader_t));
    EMFS_FileHeader_t *limit = (EMFS_FileHeader_t *)((uint8_t *)find +
                                                     EMFS_FileSystemGetPageLength(header));
...
static EMFS_FileHeader_t *EMFS_FileCreateData(const MDS_EMFS_FileSystem_t *fs,
                                              MDS_EMFS_FileId_t id, const uint8_t *data,
                                              uint16_t len)
{
    ...
    size_t splitnm = (len + sizeof(EMFS_FileHeader_t) + EMFS_FILE_SPLIT_SIZE) /
                     EMFS_FILE_SPLIT_SIZE;
    ...
    if ((fs->init.size - length) >= splitsz) {
        ...
        for (size_t idx = 0; idx < splitsz; idx++) {
            *((uint8_t *)file + sizeof(EMFS_FileHeader_t) + idx) = ((idx < len) && (data != NULL))
                                                                       ? (data[idx])
                                                                       : (0xFF);
        }
...
MDS_Err_t MDS_EMFS_Rename(MDS_EMFS_FileSystem_t *fs, MDS_EMFS_FileId_t oldid,
                          MDS_EMFS_FileId_t newid)
{
    MDS_ASSERT(fs != NULL);
...
MDS_Err_t MDS_EMFS_FileCreate(MDS_EMFS_FileDesc_t *fd, MDS_EMFS_FileSystem_t *fs,
                              MDS_EMFS_FileId_t id)
{
    MDS_ASSERT(fd != NULL);
    MDS_ASSERT(fs != NULL);
...
MDS_Err_t MDS_EMFS_FileOpen(MDS_EMFS_FileDesc_t *fd, MDS_EMFS_FileSystem_t *fs,
                            MDS_EMFS_FileId_t id)
{
    MDS_ASSERT(fd != NULL);
    MDS_ASSERT(fs != NULL);
...
MDS_Err_t MDS_EMFS_FileRead(MDS_EMFS_FileDesc_t *fd, intptr_t ofs, uint8_t *buff, size_t len,
                            size_t *read)
{
    MDS_ASSERT(fd != NULL);
    ...
    do {
        ...
        if (err != MDS_EOK) {
            MDS_LOG_E("[emfs] read file err:%d try to reload on fs:%p ofs:%u", err, fd->fs,
                      fd->fs->readOfs);
            err = EMFS_FileSystemSearchLastPage(fd->fs);
            if (err != MDS_EOK) {
                MDS_LOG_E("[emfs] read file err:%d try to research on fs:%p device:%p", err,
                          fd->fs, fd->fs->init.device);
            }
        }
    ...
    if (err == MDS_EOK) {
        ...
        count = MDS_MemBuffCopy(buff, len, (uint8_t *)file + sizeof(EMFS_FileHeader_t) + ofs,
                                used - ofs);
    }
...
MDS_Err_t MDS_EMFS_FileWrite(MDS_EMFS_FileDesc_t *fd, intptr_t ofs, const uint8_t *buff,
                             size_t len, size_t *write)
{
    ...
    do {
        if (((ofs + len) <= used) &&
            (memcmp((uint8_t *)file + sizeof(EMFS_FileHeader_t) + ofs, buff, len) == 0)) {
            err = MDS_EOK;
            break;
        }
        ...
        if ((splitsz < (len + sizeof(EMFS_FileHeader_t))) &&
            (spacesz < (len + sizeof(EMFS_FileHeader_t) - splitsz))) {
            err = MDS_ERANGE;
        } else if ((ofs + len) < splitsz) {
            count = MDS_MemBuffCopy((uint8_t *)file + sizeof(EMFS_FileHeader_t) + ofs,
                                    splitsz - ofs, buff, len);
        } else {
            MDS_MemBuffCopy((uint8_t *)(header) + pageused, spacesz, (uint8_t *)file,
                            sizeof(EMFS_FileHeader_t) + used);
            EMFS_FileRemoveData(fd->fs, file);
            file = (EMFS_FileHeader_t *)((uint8_t *)(header) + pageused -
                                         sizeof(EMFS_FileHeader_t) - used);
            count = MDS_MemBuffCopy((uint8_t *)file + sizeof(EMFS_FileHeader_t) + ofs,
                                    splitsz - ofs, buff, len);
        }
