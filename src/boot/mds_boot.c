...
static __attribute__((used,
                      section(CONFIG_MDS_BOOT_SWAP_SECTION))) MDS_BOOT_SwapInfo_t g_bootSwapInfo;
                                            ...
                                            uint32_t srcOfs, uint32_t srcSize, uint16_t flag)
{
    ...
    switch (flag) {
#if (defined(CONFIG_MDS_BOOT_UPGRADE_WITH_COPY) && (CONFIG_MDS_BOOT_UPGRADE_WITH_COPY != 0))
                                             ...
                                             const MDS_BOOT_UpgradeInfo_t *upgradeInfo)
{
    ...
    for (uint16_t i = 0; i < cnt; i++) {
        ...
        uint32_t srcSize = ALGO_GetU32BE(binInfo.srcSize);
        MDS_BOOT_Result_t result = BOOT_UpgradeSwtich(dst, src, srcOfs + sizeof(binInfo), srcSize,
                                                      flag);
        if (result != MDS_BOOT_RESULT_SUCCESS) {
            return (result);
        }
